...
hotl is a human-on-the-loop agent harness in one binary. The design commitments, concretely:
...
- **Surfaces for how you actually work.** A console TUI you can steer mid-turn, `-p` headless (with `--json`) for scripts and CI, `hotl bg`/`attach` for sessions that outlive your terminal, and `hotl watch` for supervising every agent in your tmux session.
