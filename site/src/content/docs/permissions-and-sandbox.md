...
- **Native Windows has no enforced floor yet.** hotl builds, runs and is tested there, but its write floor is written and uncertified, so `probe()` reports unavailable and Windows behaves exactly like the sub-6.2 Linux case below: every `bash` call is individually approved, every ask carries the `UNSANDBOXED` banner, and `bash` allow-rules never auto-approve. Reading, searching and editing files are unaffected — a turn that only reads and edits prompts you zero times. **WSL2 is the confined Windows path**, and gets the full Landlock floor. Two things about the designed Windows floor will not change when it is certified: the read carve is **absent** there rather than partial (the mechanism narrows only the *write* access check), and egress is permanently unenforced. [SECURITY.md](https://github.com/nrakochy/hotl/blob/master/docs/SECURITY.md#native-windows) has the full statement.
...
- **The `.git` carve-out is macOS-only.** On macOS the floor denies writes to `.git/hooks/` and `.git/config` even though both sit inside your working directory, so a command hotl ran — including one from a sub-agent working in its own git worktree — can commit but cannot install a hook or set `core.pager`/`core.fsmonitor`/`core.hooksPath` to run something later. **On Linux neither is denied**: Landlock rights union across ancestors, so carving a path back out of the working-directory grant would mean not granting the working directory at all — which measurably breaks creating any new file at its top. Linux keeps only the permission-layer escalation (the write still prompts, or notices loudly under `bypass`); the kernel is not a second line there. Same statement, same reason, as the read carve above.
