...
| `/goal [condition]` | Run until a condition is met: after each turn a small fast evaluator judges the condition against the conversation — *not yet* and the agent keeps going with the verdict's reason as guidance, *met* or *impossible* and the goal clears with a transcript notice. Both the agent and the evaluator see where the loop stands (`turn 3 · 12m elapsed · 41.2k in / 3.1k out`), so a turn or time bound written into the condition is judged against real numbers. Setting a goal while idle starts the turn at once, with the condition itself as the prompt; setting one mid-turn only arms the check for when that turn ends. `◎ /goal active · Nm` rides the status strip meanwhile. Eight consecutive turns in which no tool ran pause the loop — the goal stays set, and your next prompt re-arms it; a turn that ran a tool resets the count, so a goal making progress is never cut short. An error only you can fix (a revoked key, a 401/403, a context window compaction can no longer make room in) clears the goal instead of retrying against it; a rate limit or an outage leaves it armed. Bare `/goal` reports the condition, elapsed time, turns taken, what the loop has spent (the evaluator's own calls included) and the evaluator's last reason — and, once nothing is active, the goal that resolved last. `/goal clear` (or `stop`/`off`/`reset`/`none`/`cancel`) ends it early and echoes what it ended. There is no built-in cap on turns that make progress — bound that in the condition itself ("…or stop after 20 turns"). An active goal survives `hotl resume` (counters restart); an achieved, errored or cleared one does not. Headless: `hotl --goal "<condition>"` runs the loop in one invocation with the condition as the prompt; add `-p "<prompt>"` to open with a different first prompt. |
