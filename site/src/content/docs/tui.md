...
| `/goal [condition]` | Run until a condition is met: after each turn a small fast evaluator judges the condition against the conversation — *not yet* and the agent keeps going with the verdict's reason as guidance, *met* or *impossible* and the goal clears with a transcript notice. Setting a goal while idle starts the turn at once, with the condition itself as the prompt; setting one mid-turn only arms the check for when that turn ends. `◎ /goal active · Nm` rides the status strip meanwhile. Bare `/goal` reports the condition, elapsed time and turns taken; `/goal clear` (or `stop`/`off`/`reset`/`none`/`cancel`) ends it early. There is no built-in turn cap — bound it in the condition itself ("…or stop after 20 turns"). An active goal survives `hotl resume` (counters restart); an achieved or cleared one does not. Headless: `hotl --goal "<condition>"` runs the loop in one invocation with the condition as the prompt; add `-p "<prompt>"` to open with a different first prompt. |
