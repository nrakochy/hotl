...
- **A permission gate with a kernel floor under it.** Every mutating or executing tool call passes the gate: `auto` (default, no ordinary prompts) or `ask` (y/N per call). Independent of mode, `bash` runs confined by Seatbelt (macOS) or Landlock (Linux) to writes inside the working directory — plus any [`[sandbox].writable`](../configuration/#sandbox-write-floor-sandbox) cache directories the owner listed; entries that would expose hotl's own config are refused — and writes to execute-later paths — git hooks, shell rc, Makefiles, agent-instruction files — always prompt. A build with `--features security-enforced` cannot have prompting disabled by any config. Network egress control is opt-in: `[network].allow` routes HTTP through a local allowlist proxy, and unenforceable restrictions fail loudly, never silently.
