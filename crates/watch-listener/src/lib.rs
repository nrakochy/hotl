...
mod tests {
    ...
    impl Surface for BadSurface {
        ...
        fn focus(&self, _: &AgentObservation) -> Result<(), SurfaceError> {
            Ok(())
        }
        fn source(&self) -> Source {
            Source::Tmux
    ...
    fn snapshot_concatenates_all_surfaces() {
        let a = FakeSurface {
            items: vec![obs("a")],
            focused: RefCell::new(vec![]),
        };
        let b = FakeSurface {
            items: vec![obs("b"), obs("c")],
            focused: RefCell::new(vec![]),
        };
    ...
    fn snapshot_keeps_healthy_surfaces_when_one_fails() {
        let good = FakeSurface {
            items: vec![obs("a"), obs("b")],
            focused: RefCell::new(vec![]),
        };
        ...
        assert_eq!(
            ...
            2,
            "healthy surface's agents survive"
        );
    ...
    fn snapshot_errors_only_when_all_surfaces_fail() {
        ...
        assert!(
            l.snapshot().is_err(),
            "sole surface failing is a total outage"
        );

        let l2 = Listener::new(vec![Box::new(BadSurface("a")), Box::new(BadSurface("b"))]);
        assert!(
            l2.snapshot().is_err(),
            "every surface failing is a hard error"
        );
    }
}
