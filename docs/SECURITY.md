...
`bash` executes confined — **Seatbelt** on macOS (deny all file writes, then re-allow the working directory, temp, `/dev`, and any `[sandbox].writable` directories the owner configured), **Landlock** on Linux ≥ 6.2 (same shape; see the ABI note below). Reads stay open; network egress is open by default and restrictable per host (see "Network egress" below and "what the sandbox is not").
