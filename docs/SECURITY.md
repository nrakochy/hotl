...
**The read carve (0.10).** Reads used to be wholly open. Three tiers are now denied to every sandboxed child — the `bash` tool, `grep`'s ripgrep, post-edit diagnostic commands, and owner-configured shell hooks alike. Three of those four have no y/N prompt, which is why the config lever below exists and is not merely a convenience.
...
- **Tier C — the owner's own `[[deny]]` rules. Off unless configured, never liftable.** A `[[deny]]` over a `read`/`write`/`edit`/`glob`/`grep` path used to govern only hotl's in-process file tools: it stopped `read {"path": "/Volumes/secrets/k"}` and did nothing about `bash {"command": "cat /Volumes/secrets/k"}` — half an intent enforced. A rule naming an existing directory by an absolute or `~/`-rooted `path_prefix` is now projected onto the kernel read-deny as well. Unliftable because `Rules::evaluate` makes deny unconditional in-process, and a kernel twin that `s` could lift would reintroduce exactly the drift this closes. It is a separate field from Tier A: that one also drives the file-tool refusal message and the read-probe canary location, and hotl must not write a probe file into a directory the owner denied.

  **Projection is conservative — reported, never approximated.** A floating relative prefix (`.ssh/`, matching at any depth) has no kernel expression: the only one would be a startup walk enumerating every match on the filesystem, unbounded and blind to anything created later in the session. A command-subject rule (`[[deny]] bash prefix = "curl "`) has none either — the kernel sees paths, not command names — and that is permanent, not deferred. A `field` override, a `prefix` on a path tool, and a path that does not resolve are likewise reported. `path_prefix = "/"` and `""` are **refused loudly**: each projects to denying every read, bricking the agent in a way that looks like a hang. Everything reported is still enforced in full in-process; `hotl doctor` lists each with the form to write instead, and names the rule behind every path it does deny. `[[allow]]` rules project nothing in either direction — the only meaning on offer would lift the credential floor of every config that has one.

  **Tier C entries pass the same gauntlet Tier B does**, including the write-grant check: Landlock unions rights across ancestors, so a projected path inside cwd, `TMPDIR`, `/dev`, or a `[sandbox].writable` root cannot be denied at all, and is dropped with a warning rather than reported as live.

Two levers lift Tier B, and neither reaches Tier C. `[sandbox].readable = ["~/.ssh"]` is standing (it reaches hooks and diagnostics, and needs a session restart, since `[sandbox]` is installed once at startup); pressing `s` instead of `y` at a `bash` ask lifts it for **that one command only** — scoped around the single tool call, so it cannot leak into the next one, and unreachable headless or from a sub-agent, whose asks auto-deny. Once Tier B is lifted by config, every ask is labeled `reads:open`; the hardened default is silent.
