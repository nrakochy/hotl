...
The **default policy file** is specified in [design-docs/default-policy.md](design-docs/default-policy.md) (a minimal, read-only-by-default `config.toml` starter — "defaults are the safety design" made concrete). The two remaining **trust-prompt screens** (the MCP first-use screen shipped with M3a):
