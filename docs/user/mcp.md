...
**Mode: how-to.** Steps to give the `hotl` agent extra tools from an MCP server. Assumes you have a working agent (see [quickstart.md](quickstart.md)) and an MCP server program on your machine. For the security model behind the approval prompts, see [permissions-and-sandbox.md](permissions-and-sandbox.md).
